import { Star } from "lucide-react";
import { GOOGLE_REVIEW_URL } from "@/lib/links";

/* Google "G" logo mark */
function GoogleG({ className = "w-4 h-4" }: { className?: string }) {
  return (
    <svg viewBox="0 0 48 48" className={className} aria-hidden="true">
      <path fill="#FFC107" d="M43.611 20.083H42V20H24v8h11.303c-1.649 4.657-6.08 8-11.303 8-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4 12.955 4 4 12.955 4 24s8.955 20 20 20 20-8.955 20-20c0-1.341-.138-2.65-.389-3.917z" />
      <path fill="#FF3D00" d="M6.306 14.691l6.571 4.819C14.655 15.108 18.961 12 24 12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4 16.318 4 9.656 8.337 6.306 14.691z" />
      <path fill="#4CAF50" d="M24 44c5.166 0 9.86-1.977 13.409-5.192l-6.19-5.238C29.211 35.091 26.715 36 24 36c-5.202 0-9.619-3.317-11.283-7.946l-6.522 5.025C9.505 39.556 16.227 44 24 44z" />
      <path fill="#1976D2" d="M43.611 20.083H42V20H24v8h11.303c-.792 2.237-2.231 4.166-4.087 5.571l.003-.002 6.19 5.238C36.971 39.205 44 34 44 24c0-1.341-.138-2.65-.389-3.917z" />
    </svg>
  );
}

interface GoogleReviewButtonProps {
  /** "solid" (orange, default), "white", or "outline" */
  variant?: "solid" | "white" | "outline";
  label?: string;
  className?: string;
}

export default function GoogleReviewButton({
  variant = "solid",
  label = "Leave us a Google Review",
  className = "",
}: GoogleReviewButtonProps) {
  const base =
    "inline-flex items-center gap-2.5 font-black uppercase tracking-widest text-sm px-6 py-3.5 transition-all hover:scale-105";

  const variants: Record<string, string> = {
    solid: "bg-[#f97316] hover:bg-[#e8650f] text-white",
    white: "bg-white text-[#1a1a1a] hover:bg-gray-100 border border-gray-200",
    outline: "border-2 border-[#f97316] text-[#f97316] hover:bg-[#f97316] hover:text-white",
  };

  return (
    <a
      href={GOOGLE_REVIEW_URL}
      target="_blank"
      rel="noopener noreferrer"
      className={`${base} ${variants[variant]} ${className}`}
    >
      <span className="bg-white rounded-full p-1 flex items-center justify-center">
        <GoogleG className="w-4 h-4" />
      </span>
      {label}
      <span className="flex gap-0.5">
        {[...Array(5)].map((_, i) => (
          <Star key={i} className="w-3.5 h-3.5 fill-current" />
        ))}
      </span>
    </a>
  );
}
